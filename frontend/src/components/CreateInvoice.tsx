...
const CreateInvoice: React.FC<CreateInvoiceProps> = ({ account }) => {
  const { encryptAmount, isInitialized, initError } = useFhevm();
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="card space-y-6">
        ...
        {!isInitialized && (
          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 flex items-center space-x-3">
            ...
            <span className="text-yellow-800">Initializing FHE encryption... Please wait.</span>
          </div>
        )}

        {initError && (
          <div className="bg-red-50 border border-red-200 rounded-lg p-4">
            <p className="text-red-800">FHE Error: {initError}</p>
          </div>
        )}
